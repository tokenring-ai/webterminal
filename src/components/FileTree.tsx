...
function TreeNodeComponent({ agent, node, depth, onFileOpen, refreshTree, setError }: TreeNodeProps) {
  const team = useApp();
        ...
        {isFile ? (
          <>
            ...
            <button type="button" title="Download" onClick={handleDownload}>
              <Download size={16} className="shrink-0 text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-500 ml-1" />
            </button>
            <button type="button" title="Rename" onClick={handleRename}>
              <Edit2 size={14} className="shrink-0 text-gray-500 hover:text-blue-600 ml-1" />
            </button>
            <button type="button" title="Delete" onClick={handleDelete}>
              <Trash2 size={14} className="shrink-0 text-red-500 hover:text-red-700 ml-1" />
            </button>
          </>
        ) : (
          <>
            ...
            {open ? (
              <FolderOpen size={16} className="shrink-0 text-yellow-500 dark:text-yellow-400" />
            ) : (
              <FolderIcon size={16} className="shrink-0 text-yellow-500 dark:text-yellow-400" />
            )}
            <span className="grow truncate">{name}</span>
            <button type="button" title="Rename" onClick={handleRename}>
              <Edit2 size={14} className="shrink-0 text-gray-500 hover:text-blue-600 ml-1" />
 ...
 */
export default function FileTree({ onFileOpen, agent }: { onFileOpen: (filePath: string) => void; agent: Agent }) {
  const team = useApp();
    ...
    <>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex space-x-2">
          <button type="button" onClick={() => refreshTree()} className="p-2 rounded-lg hover:bg-gray-800 transition-colors">
      ...
      <div className="space-y-1">
        {rootNodes.map(n => (
          <TreeNodeComponent agent={agent} key={n.__full__} node={n} depth={0} onFileOpen={onFileOpen} refreshTree={refreshTree} setError={setError} />
        ))}
      </div>
    </>
  );
