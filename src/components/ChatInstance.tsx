...
const ChatInstance = ({ agentId, isActive }: { agentId: string; isActive: boolean }) => {
  ...
  const waitingOn =
    currentInputItem?.executionState.availableInteractions.find((interaction): interaction is QuestionInteraction => interaction.type === "question") ?? null;
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-gray-900">
      {waitingOn && <HumanRequestDialog request={waitingOn} onResponse={handleHumanResponse} />}
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, i) => (
          <div key={i} className={`${colorClasses[msg.type]}`}>
            {msg.message}
          </div>
        ))}
      </div>
