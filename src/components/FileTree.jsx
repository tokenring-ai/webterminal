...
import { ChevronRight, ChevronDown, FileText, Folder as FolderIcon, FolderOpen, Upload, Download, Trash2, Edit2 } from 'lucide-react';
...
function TreeNode({ node, depth, onFileOpen, refreshTree, setError }) {
  ...
  const children = childrenKeys.map(k => ({ ...node.__children__[k], __name__: k }));
      ...
      >
        {isFile ? (
          <>
            ...
            <button title="Download" onClick={handleDownload}><Download size={16} className="shrink-0 text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-500 ml-1" /></button>
            <button title="Rename" onClick={handleRename}><Edit2 size={14} className="shrink-0 text-gray-500 hover:text-blue-600 ml-1" /></button>
            <button title="Delete" onClick={handleDelete}><Trash2 size={14} className="shrink-0 text-red-500 hover:text-red-700 ml-1" /></button>
          </>
        ) : (
          <>
            {open ? <ChevronDown size={16} className="shrink-0 text-gray-500 dark:text-gray-400" /> : <ChevronRight size={16} className="shrink-0 text-gray-500 dark:text-gray-400" />}
            {open ? <FolderOpen size={16} className="shrink-0 text-yellow-500 dark:text-yellow-400" /> : <FolderIcon size={16} className="shrink-0 text-yellow-500 dark:text-yellow-400" />}
            <span className="flex-grow truncate">{name}</span>
            <button title="Rename" onClick={handleRename}><Edit2 size={14} className="shrink-0 text-gray-500 hover:text-blue-600 ml-1" /></button>
            <button title="Delete" onClick={handleDelete}><Trash2 size={14} className="shrink-0 text-red-500 hover:text-red-700 ml-1" /></button>
          </>
...
export default function FileTree({ onFileOpen }) {
  ...
  const refreshTree = useCallback(async () => {
    try {
      ...
      const paths = [];
      for await (const p of fsService.getDirectoryTree('', { ig, recursive: true })) {
        paths.push(p);
      }
  ...
  if (!treeData && !error) {
    return <div className="p-3 m-2 text-sm text-gray-700 bg-gray-100 border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-200 dark:border-gray-600">Loading file tree...</div>;
  }

  const rootNodes = Object.keys(treeData).map(k => ({ ...treeData[k], __name__: k }));
    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200">
      <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700 gap-2">
        ...
        <div className="flex gap-2">
          <button
            onClick={async () => {
              const fileName = prompt('Enter new file name (with path if needed):');
              if (!fileName) return;
              try {
                const fsService = registry.getFirstServiceByType(FileSystemService);
          ...
          <button
            onClick={async () => {
              const folderName = prompt('Enter new folder name (with path if needed):');
