...
export default function MainPanel({ agent, activeView, tabs, onFileOpen, onFileClose, onNewChat, onSelectChatTab }: MainPanelProps) {
    ...
    <div className="h-full flex flex-col bg-white dark:bg-gray-900 overflow-hidden">
      {activeView === "agents" && (
        <div className="flex-1 overflow-y-auto p-4">
          <ChatList chatTabs={tabs.filter(t => t.type === "chat")} onNewChat={onNewChat} onSelectChatTab={onSelectChatTab} />
        </div>
      )}
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {fileTab && (
            <div className="flex-1 flex flex-col overflow-hidden relative">
              ...
              <FileViewer agent={agent} filePath={fileTab.filePath!} onFileDeleted={() => onFileClose(fileTab.id)} onFileRenamed={() => {}} />
            </div>
          )}
        </div>
      )}
