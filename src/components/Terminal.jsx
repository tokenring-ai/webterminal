...
function Terminal() {
 ...
 // Added historyIndex
 const { chunks, handleInput, getPreviousCommand, getNextCommand, historyIndex } = useRepl(registry);

 const [input, setInput] = useState('');
 const [currentInputValueBeforeHistory, setCurrentInputValueBeforeHistory] = useState(''); // Store input before navigating history
 ...
 const handleKeyDown = (e) => {
  if (e.key === 'ArrowUp') {
    e.preventDefault();
    if (historyIndex === -1) { // If on the current input line (not browsing history yet)
    ...
    if (historyIndex === -1 && nextCommand === '') { // Correctly check if we should restore the original input
