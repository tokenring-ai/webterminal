...
const SettingsSection = ({ title, children }) => (
  <section className="mb-8 p-4 bg-white dark:bg-gray-800 shadow-md rounded-lg">
    <h4 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300 border-b pb-2 dark:border-gray-700">{title}</h4>
...
const ActionButton = ({ onClick, children, variant = 'primary', size = 'sm', className = '' }) => {
  const baseStyle = "px-3 py-1.5 rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800";
  const sizeStyles = {
    sm: "text-xs",
    md: "text-sm",
  };
...
export default function SettingsPanel() {
  ...
  const { config, setConfigurationValue, getConfigurationValue } = useConfigurationStore();
  ...
  const removeDefaultFile = (indexToRemove) => () => {
    const currentFiles = getConfigurationValue('defaults.selectedFiles') || [];
    setConfigurationValue('defaults.selectedFiles', currentFiles.filter((_, i) => i !== indexToRemove));
  };
  ...
  const handleRemoveDefaultTool = (indexToRemove) => {
    const currentTools = getConfigurationValue('defaults.tools') || [];
    setConfigurationValue('defaults.tools', currentTools.filter((_, i) => i !== indexToRemove));
  };
  ...
  const handleAddModelConfiguration = () => {
    const newModelKey = window.prompt("Enter a unique key for the new model configuration (e.g., 'MyGroqLlama'):");
  ...
  const handleRemoveModelConfiguration = (modelKeyToRemove) => {
    if (window.confirm(`Are you sure you want to remove the model configuration '${modelKeyToRemove}'?`)) {
      const currentModels = getConfigurationValue('models') || {};
  ...
  const handleAddPersona = () => {
    const newPersonaKey = window.prompt("Enter a name for the new persona (e.g., 'helpful-assistant'):");
  ...
  const handleRemovePersona = (personaKey) => {
    if (window.confirm(`Are you sure you want to remove the persona '${personaKey}'?`)) {
      const currentPersonas = getConfigurationValue('personas') || {};
  ...
  const addIndexed = () => {
    const currentIndexedFiles = getConfigurationValue('indexedFiles') || [];
    setConfigurationValue('indexedFiles', [...currentIndexedFiles, { path: '' }]);
  };

  const removeIndexed = (indexToRemove) => () => {
    const currentIndexedFiles = getConfigurationValue('indexedFiles') || [];
    setConfigurationValue('indexedFiles', currentIndexedFiles.filter((_, i) => i !== indexToRemove));
  };
  ...
  const addWatched = () => {
    const currentWatchedFiles = getConfigurationValue('watchedFiles') || [];
    setConfigurationValue('watchedFiles', [...currentWatchedFiles, { path: '', include: '' }]);
  };

  const removeWatched = (indexToRemove) => () => {
    const currentWatchedFiles = getConfigurationValue('watchedFiles') || [];
    setConfigurationValue('watchedFiles', currentWatchedFiles.filter((_, i) => i !== indexToRemove));
  };

  // Handlers for resources
  const handleAddResourceGroup = () => {
    const newResourceKey = window.prompt("Enter a unique key for the new resource group (e.g., 'projectSrc'):");
    if (newResourceKey && newResourceKey.trim() !== "") {
      ...
      setConfigurationValue(`resources.${newResourceKey}`, newResourceGroupDefaults);
    }
  };

  const handleRemoveResourceGroup = (resourceKeyToRemove) => {
    if (window.confirm(`Are you sure you want to remove the resource group '${resourceKeyToRemove}' and all its items?`)) {
      const currentResources = getConfigurationValue('resources') || {};
      const { [resourceKeyToRemove]: _, ...remainingResources } = currentResources;
  ...
  const handleAddResourceItem = (resourceKey) => {
    const itemsPath = `resources.${resourceKey}.items`;
    const currentItems = getConfigurationValue(itemsPath) || [];
    setConfigurationValue(itemsPath, [...currentItems, { path: '', include: '' }]);
  };
  ...
  return (
    <div className="p-4 space-y-6 overflow-auto text-sm bg-gray-50 dark:bg-gray-900 h-full">
      ...
      <SettingsSection title="Defaults">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Default Persona</label>
          <StyledSelect
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Default Resources</label>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-2 border dark:border-gray-700 rounded-md">
            {Object.keys(config?.resources ?? {}).map(resKey => (
              <label key={resKey} className="flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-300 p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  checked={config?.defaults?.resources?.includes(resKey) || false}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Default Selected Files</label>
          <div className="space-y-2">
            {config?.defaults?.selectedFiles?.map((f,i) => (
              <div key={i} className="flex items-center space-x-2">
                ...
                />
                <ActionButton onClick={removeDefaultFile(i)} variant="danger" size="sm" className="p-1.5">
                  <Trash2 size={14}/>
                </ActionButton>
              </div>
            ))}
            <ActionButton onClick={addDefaultFile} variant="secondary" size="sm" className="flex items-center gap-1">
              <PlusCircle size={14}/> Add File
            </ActionButton>
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Default Tools</label>
          <div className="space-y-2">
            {(config?.defaults?.tools || []).map((toolName, index) => (
              <div key={index} className="flex items-center space-x-2">
                <StyledInput
                  type="text"
                  value={toolName}
                  onChange={(e) => handleDefaultToolChange(index, e.target.value)}
                  placeholder="Enter tool name (e.g., readFile)"
                />
                <ActionButton onClick={() => handleRemoveDefaultTool(index)} variant="danger" size="sm" className="p-1.5">
                   <Trash2 size={14}/>
                </ActionButton>
              </div>
            ))}
            <ActionButton onClick={handleAddDefaultTool} variant="secondary" size="sm" className="flex items-center gap-1">
      ...
      <SettingsSection title="Model Configurations">
        <div className="space-y-4">
          {Object.entries(config?.models ?? {}).map(([modelKey, modelConfig]) => (
            <div key={modelKey} className="p-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-850 shadow">
              <div className="flex justify-between items-center mb-3">
                <h5 className="font-medium text-md text-gray-700 dark:text-gray-300">{modelKey}</h5>
                <ActionButton onClick={() => handleRemoveModelConfiguration(modelKey)} variant="danger" size="sm">
                  Remove Model
                </ActionButton>
              </div>
          ...
          <ActionButton onClick={handleAddModelConfiguration} variant="success" size="md" className="flex items-center gap-1">
      ...
      <SettingsSection title="Personas">
        <div className="space-y-4">
          {Object.entries(config?.personas ?? {}).map(([personaKey, personaConfig]) => (
            <div key={personaKey} className="p-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-850 shadow">
              <div className="flex justify-between items-center mb-3">
                <h5 className="font-medium text-md text-gray-700 dark:text-gray-300">{personaKey}</h5>
                <ActionButton onClick={() => handleRemovePersona(personaKey)} variant="danger" size="sm">
                  Remove Persona
                </ActionButton>
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <StyledInput
                    ...
                    onChange={(e) => handlePersonaFieldChange(personaKey, 'model', e.target.value)}
                ...
                <div>
                  ...
                  <StyledTextArea
                    ...
                    onChange={(e) => handlePersonaFieldChange(personaKey, 'instructions', e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <StyledInput
                      ...
                      onChange={(e) => handlePersonaFieldChange(personaKey, 'temperature', e.target.value)}
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Top P</label>
                    <StyledInput
                      type="number"
                      step="0.05"
                      min="0"
                      ...
                      onChange={(e) => handlePersonaFieldChange(personaKey, 'top_p', e.target.value)}
                    />
                  </div>
                </div>
              </div>
          ...
          <ActionButton onClick={handleAddPersona} variant="success" size="md" className="flex items-center gap-1">
        ...
        <div className="space-y-2">
          {config?.indexedFiles?.map((item,i) => (
            <div key={i} className="flex items-center space-x-2">
              ...
              />
              <ActionButton onClick={removeIndexed(i)} variant="danger" size="sm" className="p-1.5">
                <Trash2 size={14}/>
              </ActionButton>
            </div>
          ))}
          <ActionButton onClick={addIndexed} variant="secondary" size="sm" className="flex items-center gap-1">
        ...
        <div className="space-y-2">
          {config?.watchedFiles?.map((wf,i) => (
            <div key={i} className="flex items-center space-x-2">
              ...
              />
              <ActionButton onClick={removeWatched(i)} variant="danger" size="sm" className="p-1.5">
                <Trash2 size={14}/>
              </ActionButton>
            </div>
          ))}
          <ActionButton onClick={addWatched} variant="secondary" size="sm" className="flex items-center gap-1">
      ...
      <SettingsSection title="Resource Definitions">
        <div className="space-y-4">
          {Object.entries(config?.resources ?? {}).map(([resourceKey, resourceValue]) => (
            <div key={resourceKey} className="p-3 border border-gray-200 dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-850 shadow">
              <div className="flex justify-between items-center mb-3">
                <h5 className="font-medium text-md text-gray-700 dark:text-gray-300">{resourceKey}</h5>
                <ActionButton onClick={() => handleRemoveResourceGroup(resourceKey)} variant="danger" size="sm">
                  Remove Group
                </ActionButton>
              </div>
              <div className="mb-3">
                ...
                <StyledSelect
                  ...
                  onChange={(e) => handleResourceGroupTypeChange(resourceKey, e.target.value)}
                >
                  {['fileTree', 'wholeFile', 'repoMap', 'testing', 'symbol', 'search'].map(type => ( // Added more common types
              ...
              <div>
                <h6 className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">Items:</h6>
                <div className="space-y-2 pl-3 border-l-2 border-gray-200 dark:border-gray-700">
                  {(resourceValue.items || []).map((item, itemIndex) => (
                    <div key={itemIndex} className="p-2 rounded-md bg-gray-100 dark:bg-gray-750 border border-gray-200 dark:border-gray-600">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-1">
                         <StyledInput
                          type="text"
                          placeholder="Path (e.g., ./src, file.js)"
                          value={item.path || ''}
                          onChange={(e) => handleResourceItemChange(resourceKey, itemIndex, 'path', e.target.value)}
                        />
                        <StyledInput
                          type="text"
                          placeholder="Include Regex (optional)"
                          value={item.include || ''}
                          onChange={(e) => handleResourceItemChange(resourceKey, itemIndex, 'include', e.target.value)}
                        />
                      </div>
                      <ActionButton
                        onClick={() => handleRemoveResourceItem(resourceKey, itemIndex)}
                        variant="danger"
                        size="sm"
          ...
          <ActionButton onClick={handleAddResourceGroup} variant="success" size="md" className="flex items-center gap-1">
