  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <>
          <span className="px-2 py-1 text-xs text-gray-800 dark:text-gray-200" title={session.user?.email}>{session.user?.name || session.user?.email}</span>
          <button onClick={signOut} className="px-2 py-1 text-xs" title="Sign out">Sign out</button>
        </>
      ) : (
        <button onClick={signIn} className="px-2 py-1 text-xs" title="Sign in">Sign in</button>
      )}
      <button
