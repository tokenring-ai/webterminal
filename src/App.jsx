...
function App() {
  ...
  const globalConfigService = globalRegistry?.requireFirstServiceByType(ConfigurationManagementService);
  ...
  const handleNewChat = useCallback(() => {
    ...
    if (globalConfigService) {
      ...
      initialConfigForNewChat = JSON.parse(JSON.stringify(globalConfigService.getConfiguration()));
    } else {
      // Fallback if globalConfigService is somehow not available (should not happen in normal flow)
      console.warn("Global ConfigurationManagementService not found. Initializing new chat with local defaultChatConfig.");
    ...
    const newConfigService = new ConfigurationManagementService(initialConfigForNewChat);
  ...
  const openChatTab = useCallback((chatId, title) => {
    ...
    setTabs(prev => {
      ...
      console.warn("Legacy openChatTab called, may not work with new ChatInstance expecting configService");
      return prev;
    });
    // setActiveTabId(id);
  }, []);
        ...
        <div className="h-full flex flex-col">
          ...
          >
            <TopBar
              ...
              onResize={delta => setHeight(h => Math.min(window.innerHeight, Math.max(200, h - delta)))}
            />
