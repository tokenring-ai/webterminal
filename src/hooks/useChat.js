	...
	useEffect(() => {
		...
		const initialize = async () => {
			try {
				...
				if (!chatInstanceId) {
					console.warn(
						"useChat: No 'id' or 'chatId' found in configServiceInstance's configuration. Browser storage services might not be properly isolated.",
					);
				...
				console.log(
					`useChat: Initializing chat environment with config from service. Instance ID for storage: ${chatInstanceId}`,
					actualConfig,
				);
				...
				await modelRegistry.initializeModels(
					{ ...models, ...currentModelsConfig.libraryModels },
					currentModelsConfig,
				);
				...
				await newRegistry.services.addServices(
					...
					new BrowserChatMessageStorage({
						storagePrefix: chatInstanceId
							? `${chatInstanceId}_messages_`
							: undefined,
					}),
				...
				await newRegistry.tools.enableTools(
					currentDefaults.tools || defaultToolsList,
				);
