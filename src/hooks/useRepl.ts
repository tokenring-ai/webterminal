...
export function useRepl(agent: Agent | null) {
  ...
  const handleInput = useCallback(
    (line: string) => {
      ...
      if (processedInput && (currentHistory.length === 0 || currentHistory[currentHistory.length - 1] !== processedInput)) {
  ...
  const getPreviousCommand = useCallback(() => {
    ...
    const newIndex = currentIndex === -1 ? currentHistory.length - 1 : Math.max(0, currentIndex - 1);
